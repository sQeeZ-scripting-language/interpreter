...
Object::keys(std::string method, Expr *caller, Storage::DataWrapper callerValue,
  ...
  for (const auto &entry : *callerValue.data._object) {
    keys.push_back(Storage::DataWrapper(Storage::WrapperType::VALUE,
                                        Storage::DataType::STRING,
                                        new std::string(entry.first)));
  }
...
Object::values(std::string method, Expr *caller,
  ...
  std::vector<Storage::DataWrapper> keys;
  for (const auto &entry : *callerValue.data._object) {
    keys.push_back(Storage::DataWrapper(
        Storage::WrapperType::VALUE, entry.second.dataType, entry.second.data));
  }
  return Storage::DataWrapper(Storage::WrapperType::VALUE,
                              Storage::DataType::ARRAY,
                              new std::vector<Storage::DataWrapper>(keys));
...
Object::entries(std::string method, Expr *caller,
  ...
  for (const auto &entry : *callerValue.data._object) {
    ...
    entryData.push_back(Storage::DataWrapper(Storage::WrapperType::VALUE,
                                             Storage::DataType::STRING,
                                             new std::string(entry.first)));
    entryData.push_back(Storage::DataWrapper(
        Storage::WrapperType::VALUE, entry.second.dataType, entry.second.data));
    keys.push_back(Storage::DataWrapper(
        Storage::WrapperType::VALUE, Storage::DataType::ARRAY,
        new std::vector<Storage::DataWrapper>(entryData)));
  }
  return Storage::DataWrapper(Storage::WrapperType::VALUE,
                              Storage::DataType::ARRAY,
                              new std::vector<Storage::DataWrapper>(keys));
...
Object::get(std::string method, Expr *caller, Storage::DataWrapper callerValue,
  ...
  return Storage::DataWrapper(Storage::WrapperType::VALUE, it->second.dataType,
                              it->second.data);
}
