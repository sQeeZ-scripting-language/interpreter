...
InterpreterNode::InterpreterNode(const Napi::CallbackInfo &args)
    : ObjectWrap(args) {
  this->_env = args.Env();
  if (args.Length() < 1 || !args[0].IsFunction()) {
    Napi::TypeError::New(this->_env, "Function expected")
  ...
  if (!std::regex_search(require.ToString().Utf8Value().c_str(), self_regex)) {
    Napi::TypeError::New(this->_env, "{require} Function expected")
        .ThrowAsJavaScriptException();
  }
...
Napi::Array InterpreterNode::interpret(const Napi::CallbackInfo &args) {
  ...
  if (program == nullptr) {
    Napi::TypeError::New(env, "Failed to parse the code!")
        .ThrowAsJavaScriptException();
  }
...
Napi::Function InterpreterNode::GetClass(Napi::Env env) {
  return DefineClass(
      env, "InterpreterNode",
      {InterpreterNode::InstanceMethod("pingInstance",
                                       reinterpret_cast<InstanceMethodCallback>(
                                           &InterpreterNode::pingInstance)),
       InterpreterNode::InstanceMethod("interpret",
                                       reinterpret_cast<InstanceMethodCallback>(
                                           &InterpreterNode::interpret))});
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  exports.Set(Napi::String::New(env, "pingInterpreter"),
              Napi::Function::New(env, pingInterpreter));
